...
export default function ArticleDetailPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto bg-white rounded-lg shadow p-8">
      ...
      {subscribed ? (
        <p className="text-neutral-800 whitespace-pre-line">{article.content}</p>
      ) : (
        <>
          <p className="text-neutral-700 mb-4">{article.preview}</p>
      ...
      <div className="mt-8">
        <button onClick={() => router.back()} className="text-neutral-600 hover:underline">&larr; Back to articles</button>
      </div>
    </div>
  );
} 